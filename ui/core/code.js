...
Code.renderContent = function() {
  ...
  if (content.id == 'content_code') {
    ...
    xmlTextarea.innerText = UI ['workspace'].writeWorkspace(xmlText, true);
...
Code.init = function() {
  ...
  Code.bindClick('editAsFileButton',
    function () {
      let code = document.getElementById ('content_python').innerText;
      Files.editor.getDoc().setValue(code);

      let desc = code.match(/#Description: '(.*)'/)

      let file_name = desc ? desc [1].replaceAll(' ', '_').replaceAll('.', '')  + ".py" : "BIPES_" + + new Date() + ".py";
